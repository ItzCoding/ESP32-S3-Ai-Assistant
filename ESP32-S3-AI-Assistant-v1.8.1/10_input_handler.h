...
static void listTasksNatural() {
  listStructuredTasks(false);
}

static bool completeTask(int displayIndex) {
  return finishStructuredTask(displayIndex, "");
}
...
void handleInput(const String& input) {
  ...
  // Prefer natural phrases for daily use; slash commands remain compatible.
  if (handleSmartNaturalInput(input)) return;

  if      (lower == "/help")    { printHelp(); }
  else if (lower == "/health" || lower == "/storage") { printHardwareHealth(); }
  else if (lower == "/save") {
    flushDirtyState();
    bool pending = g_dirtyMemory || g_dirtyReminders || g_dirtyChat ||
      g_dirtyPattern || g_dirtySentiment || g_dirtyKnowledge || g_dirtySkills ||
      g_dirtyTasks || g_dirtyApiStats || g_dirtySearchCache;
    Serial.println(!g_storageReady ? "Storage offline; changes remain in RAM." :
      pending ? "Some saves failed; automatic retry remains pending." : "All changes saved.");
  }
  else if (input == "/storage format CONFIRM") {
    FFat.end();
    g_storageReady = false;
    if (FFat.format()) g_storageReady = FFat.begin(false);
    if (g_storageReady) {
      g_dirtyMemory = g_dirtyReminders = g_dirtyChat = g_dirtyPattern = true;
      g_dirtySentiment = g_dirtyKnowledge = g_dirtySkills = true;
      g_dirtyTasks = g_dirtyApiStats = g_dirtySearchCache = true;
      flushDirtyState();
    }
    Serial.println(g_storageReady ? "Storage initialized. Use /save to check saved state." : "Storage initialization failed.");
  }
  else if (lower.startsWith("/storage ")) {
    Serial.println("To erase stored files and save current RAM state: /storage format CONFIRM");
  }
    ...
    if (task.isEmpty()) Serial.println("📝 Tell me what task you want to add.");
      ...
      TaskItem item; item.id=g_nextTaskId++; item.title=task; item.createdAt=timeStatus()==timeNotSet?0:(uint32_t)now();
      tasks.push_back(item); g_dirtyTasks=true;
    ...
    if (!numeric) {
      Serial.println("❌ Usage: /remove [number]   (say 'show my reminders' to see numbers)");
    } else {
      int shownNumber = arg.toInt();
      // Lists are one-based. Keep /remove 0 as a compatibility alias for the first item.
      int idx = shownNumber == 0 ? 0 : shownNumber - 1;
      if (idx >= 0 && idx < (int)reminders.size()) {
        Serial.println("✅ Removed: " + reminders[idx].message);
        removeReminder(idx);
      } else {
        Serial.println("❌ Invalid number. Say 'show my reminders' to see the list.");
      }
    }
  }
    ...
    if (memory.empty()) { Serial.println("📭 No facts stored yet."); }
      ...
      for (const auto& f : memory) {
        String expiry = f.expiresAt ? "; expires " + formatDateTime(f.expiresAt) : "";
        Serial.println("  • " + f.key + " = " + f.value + "  [accessed " +
                       String(f.accessCount) + "x" + expiry + "]");
      }
      Serial.println("══════════════════════════");
    }
  }
  ...
  else if (lower == "/usage")   { printApiUsageStats(); }
  ...
  // ── v1.8.0 new commands ─────────────────────────────
    ...
    // that contain spaces (e.g. "Dialog 4G 188") parse correctly.
    ...
    if (sp < 0) {
      ...
      if (svc == "ai") {
        ...
        Serial.println("✅ OpenRouter AI key saved. Automatic multi-model routing is ready.");
      } else if (svc == "skill") {
        g_skillKey = key;
