...
// ║  Skills  : Gemini generates · Groq verifies · feedback loop             ║
// ║  v1.7.9  : Updated Gemini model (2.5-flash-lite → 3.1-flash-lite)       ║
// ║  v1.7.9  : Fixed WDT "task not found" crash in OTA background task      ║
// ║  v1.7.9  : Fixed AI greeting repetition comment in system prompt        ║
// ║  v1.7.9  : IMPROVED — groqVerifySkill context-aware and checks trigger  ║
// ║            coverage (short words like "stop"/"reset"/"check" required)  ║
// ║  Consoles: https://console.groq.com/keys · https://aistudio.google.com  ║
...
namespace Config {
  ...
  // ── Groq API ────────────────────────────────────────
  constexpr const char* GROQ_KEY      = "YOUR_GROQ_API_KEY";           // console.groq.com/keys
  ...
  // Reduced to 35 s — safely under the watchdog, still long enough for Gemini.
  ...
  // ── Self-taught Skills Engine (Gemini) ──────────────
  constexpr const char* GEMINI_API_KEY  = "YOUR_GEMINI_API_KEY";       // aistudio.google.com
  constexpr const char* GEMINI_MODEL    = "gemini-3.1-flash-lite";
  constexpr const char* GEMINI_ENDPOINT = "https://generativelanguage.googleapis.com/v1beta/models/";
  ...
  constexpr const char* FIRMWARE_VERSION     = "1.7.9";
  constexpr int         OTA_CHECK_TIMEOUT_MS = 12000;

  constexpr const char* VERSION = "ESP32-AI v1.7.9-GEMINI (Llama 3.3 70B + Gemini skills)";
}
...
String groqVerifySkill(const String& candidateJson, const String& request); // v1.7.9
...
void loadRuntimeSettings() {
  ...
  g_geminiKey    = g_preferences.getString("gemini_key", Config::GEMINI_API_KEY);
  customApiKey   = g_groqKey;

  // A one-shot marker is written only by HTTP_UPDATE_OK. Consume it before
  // opening the wizard. It remains set until the wizard saves successfully,
  // so an interrupted setup can continue after a reboot.
  g_setupPending = g_preferences.getBool(OTA_SETUP_ONCE_KEY, false);
  // Remove markers written by older revisions. They are never valid triggers.
...
void setup() {
  ...
  // Dedicated LED task (Core 1, priority 2)
  xTaskCreatePinnedToCore(ledTaskFn, "led", 3072, nullptr, 2, &g_ledTask, 1);
  if (g_ledTask) Serial.println("✅ LED task on Core 1");
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      if (inputString.length() > 0) {
        ...
        inputString = "";
        if (!xQueueSend(g_inputQueue, &p, 0)) delete p; // drop if queue full
      }
    } else if (c != '\r') {
      inputString += c;
    }
  ...
  // ── Process one queued input per loop iteration ───────
  {
    ...
    if (xQueueReceive(g_inputQueue, &pending, 0) == pdTRUE && pending) {
      pending->trim();
...
String buildSystemPrompt() {
  ...
  // Identity
  prompt  = "You are ESP32-AI v1.7.9, an embedded personal AI assistant running on an ESP32-S3 microcontroller. ";
...
//   3. Groq micro-call for ambiguous cases (cached — won't repeat if unchanged)
String detectLanguage(const String& msg) {
  ...
  // ── 3. All-ASCII or unrecognised → keep current / use Groq ──
  ...
  if (hasNonAscii && heapOk() && WiFi.status() == WL_CONNECTED) {
    String prompt =
      ...
      + msg.substring(0, 80) + "\"";
    String lang = groqSimpleCall(prompt, 0.0f, 10);
    lang.trim();
...
// ═══════════════════════════════════════════════════════
// SECTION 18 ── CHAT HISTORY  (v1.7.8: AI summarization)
// ═══════════════════════════════════════════════════════

int estimateTokens(const char* text) { return strlen(text) / 4; }

// v1.7.8: True AI-powered summarization — replaces crude string concatenation
void summarizeChatHistory() {
...
void printVersion() {
  ...
  Serial.println("Dual-core: " + (g_dcTask ? String("Core 0 HTTP worker") : String("single-core")));
...
{"name":"word_of_day","description":"Fetch a word of the day","vars":{},"strvars":{"word":""},"triggers":{"get":["word of the day","give me a new word","daily word","word"]},"actions":{"get":[{"op":"groq","prompt":"Give me one interesting English word with its definition in one sentence.","strvar":"word"},{"op":"say","text":"Word of the day: {word}"}]}}
)PROMPT";

// ── Skill writer — powered by Gemini ─────────────────────
// v1.7.9: Injects existing skill context so Gemini resolves ambiguous commands.
//   e.g. "stop" with a stopwatch loaded → Gemini generates a stop action for
...
String geminiGenerateSkill(const String& request, const String& previousSkillJson, const String& feedback) {
  ...
  // Gemini uses the generateContent schema, not the OpenAI-compatible
  // messages/choices schema used by the main Groq chat engine.
  JsonDocument reqDoc;
  ...
  reqDoc["generationConfig"]["temperature"] = 0.2;
  reqDoc["generationConfig"]["maxOutputTokens"] = 900;
  reqDoc["generationConfig"]["responseMimeType"] = "application/json";

  String body; serializeJson(reqDoc, body);

  setCpuFrequencyMhz(Config::CPU_FREQ_ACTIVE);
  String geminiUrl = String(Config::GEMINI_ENDPOINT) + Config::GEMINI_MODEL +
                     ":generateContent?key=" + g_geminiKey;
  String respRaw = postJson(geminiUrl, body);
  setCpuFrequencyMhz(Config::CPU_FREQ_IDLE);

  if (respRaw.length() == 0) {
    Serial.println("⚠️  Skill generation: no response from Gemini.");
    return "";
  }
  ...
  if (parseErr) {
    Serial.println("⚠️  Skill generation: unparseable Gemini response (" +
  ...
  if (strlen(apiErr) > 0) {
    Serial.println("⚠️  Gemini error: " + String(apiErr));
    return "";
  }

  JsonArrayConst candidates = resp["candidates"].as<JsonArrayConst>();
  if (candidates.isNull() || candidates.size() == 0) {
    const char* reason = resp["promptFeedback"]["blockReason"]|"";
    if (strlen(reason) > 0)
      Serial.println("⚠️  Gemini blocked skill generation: " + String(reason));
    else
      Serial.println("⚠️  Gemini returned no skill candidate.");
    return "";
  }

  String raw;
  JsonArrayConst parts = resp["candidates"][0]["content"]["parts"].as<JsonArrayConst>();
  ...
  raw.trim();
  if (raw.length() == 0) {
    Serial.println("⚠️  Gemini returned an empty skill response.");
    return "";
  }
...
String groqVerifySkill(const String& candidateJson, const String& request) {
  ...
  String verdict = groqSimpleCall(prompt, 0.0f, 280);
  verdict.trim();
...
void learnNewSkill(const String& request) {
  ...
  if (candidate.length() == 0) {
    Serial.println("❌ Couldn't generate skill (Gemini error or bad response).");
    aiState = AI_ERROR; blinkCount = 0; lastBlink = millis(); return;
  }

  // ── Gemini→Groq→Gemini verification loop (up to 2 correction rounds) ──
  for (int round = 0; round < 2; round++) {
    Serial.println("🔍 Groq verifying skill (round " + String(round + 1) + ")...");
    String feedback = groqVerifySkill(candidate, request);
    if (feedback.length() == 0) {
      Serial.println("✅ Groq verification passed.");
      break; // skill is good — proceed
    }
    // Groq found issues — log them and send back to Gemini for correction
    Serial.println("⚠️  Groq found issues: " +
                   feedback.substring(0, min((int)feedback.length(), 100)) +
                   (feedback.length() > 100 ? "..." : ""));
    Serial.println("🔄 Sending feedback to Gemini for correction...");
    String revised = geminiGenerateSkill(request, candidate, feedback);
    if (revised.length() == 0) {
      Serial.println("⚠️  Gemini correction failed — using previous candidate.");
      break; // use whatever we had
    }
    candidate = revised;
    // After the last round we use whatever Gemini produced (no third Groq check)
  }
...
// ═══════════════════════════════════════════════════════
// RELEASE WORKFLOW — only ONE thing to change per release:
//   1. Bump FIRMWARE_VERSION above, e.g. "1.7.9"
//   2. Compile → Sketch → Export Compiled Binary in Arduino IDE
//   3. Attach the .bin to a GitHub Release tagged "v1.7.9" — any filename works
//   4. Flash this updated source once via USB — OTA handles all future updates
// ────────────────────────────────────────────────────────
...
static bool otaParseRelease(const String& json, String& outVer, String& outUrl, String& outNotes) {
  ...
  // ── find ANY .bin asset — name doesn't matter ────────
  int searchFrom = 0;
  while (true) {
    ...
    if (uq1>0 && uq2>uq1) {
      ...
      if (url.endsWith(".bin")) { outUrl = url; return true; }
    }
    searchFrom = ui + 1;
  }
}
