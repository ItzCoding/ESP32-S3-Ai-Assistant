// ╔══════════════════════════════════════════════════════════════════╗
// ║               ESP32-S3-AI Assistant  v1.7.6-GROQ                    ║
...
// ╠══════════════════════════════════════════════════════════════════╣
// ║  UPGRADES v1.7.6                                                    ║
// ║  [1] PSRAM buffers for HTTP request/response bodies               ║
// ║  [2] Persistent temperature sensor handle (init once)            ║
// ║  [3] Dual-core: AI HTTP on Core 0, loop/LED/reminders Core 1     ║
// ║  [4] Advanced Skills DSL (loop, groq, remember, recall, set_str) ║
// ║  [5] Quality: summarize-once, batched flash, non-blocking WiFi   ║
// ╚══════════════════════════════════════════════════════════════════╝
//
...
#ifdef CONFIG_SPIRAM
#include <esp_psram.h>                           // [1] PSRAM detection (only if PSRAM enabled in IDE)
#endif
...
namespace Config {
  ...
  constexpr size_t CHAT_MSG_LEN     = 1200;    // [5] increased from 600
  ...
  constexpr int    MAX_SKILL_VARS         = 12;   // [4] increased from 8
  constexpr int    MAX_SKILL_STR_VARS     = 4;    // [4] new: string vars
  constexpr int    MAX_SKILL_ACTIONS      = 8;    // [4] increased from 6
  constexpr int    MAX_OPS_PER_ACTION     = 16;   // [4] increased from 12
  constexpr int    MAX_SKILL_WAIT_MS      = 3000;
  constexpr size_t MAX_SKILL_JSON_BYTES   = 4096; // [4] increased from 3072
  constexpr int    MAX_IF_DEPTH           = 3;    // [4] increased from 2
  constexpr int    MAX_LOOP_COUNT         = 10;   // [4] new: max loop iterations

  // [1] PSRAM buffer sizes
  constexpr size_t PSRAM_REQ_SIZE  = 8192;   // request body serialization buffer
  constexpr size_t PSRAM_RESP_SIZE = 16384;  // response body receive buffer

  constexpr const char* VERSION = "ESP32-AI v1.7.6-GROQ (Llama 3.1 8B)";
}
...
String buildSystemPrompt() {
  ...
  prompt  = "You are ESP32-S3-AI v1.7.6, an intelligent embedded assistant running on an ESP32-S3 microcontroller.\n";
...
// ═══════════════════════════════════════════════════════
//
// Advanced DSL additions in v1.7.6:
