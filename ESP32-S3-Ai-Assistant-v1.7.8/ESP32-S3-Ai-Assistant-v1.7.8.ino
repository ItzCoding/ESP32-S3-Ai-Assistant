// ╔══════════════════════════════════════════════════════════════════╗
// ║               ESP32-S3-AI Assistant  v1.7.8-GROQ                 ║
...
// ║  v1.7.8  : Mood-adaptive temp · CPU scaling · Groq fn calls      ║
// ║  v1.7.8  : OTA auto-update via GitHub Releases                   ║
...
namespace Config {
  ...
  // ── Groq API ────────────────────────────────────────
  // v1.7.8: Upgraded to llama-3.3-70b-versatile (best on Groq)
  ...
  // Checks https://github.com/ItzCoding/ESP32-S3-Ai-Assistant for new releases.
  // Asset name is auto-derived: ESP32-S3-AI-Assistant-v<FIRMWARE_VERSION>.bin
  ...
  constexpr const char* FIRMWARE_VERSION     = "1.7.8";
  constexpr int         OTA_CHECK_TIMEOUT_MS = 12000;

  constexpr const char* VERSION = "ESP32-AI v1.7.8-GROQ (Llama 3.3 70B)";
}
...
void loop() {
  ...
  // v1.7.8: Non-blocking WiFi reconnect
  ...
  // v1.7.8: Evening summary at 20:00
  ...
  // v1.7.8: Batch FATFS writes — flush dirty state every 30s
...
String buildSystemPrompt() {
  ...
  // Identity
  prompt  = "You are ESP32-AI v1.7.8, an embedded personal AI assistant running on an ESP32-S3 microcontroller. ";
...
void printVersion() {
  ...
  Serial.println("Model:     " + String(Config::GROQ_MODEL));
  Serial.println("CPU freq:  " + String(getCpuFrequencyMhz()) + " MHz");  // v1.7.8
  Serial.println("Mood temp: " + String(computeMoodTemperature(), 2));     // v1.7.8
...
//   3. Rename the .bin to  ESP32-S3-AI-Assistant-v1.7.9.bin  (matches auto-derived name)
//   4. Create GitHub Release tagged "v1.7.9", attach that .bin, publish
//   5. Flash this updated source once via USB — OTA handles all future updates
// ────────────────────────────────────────────────────────

// Returns the expected .bin asset name for the CURRENT firmware version.
// Pattern: ESP32-S3-AI-Assistant-v<FIRMWARE_VERSION>.bin
static String otaBinFilename() {
  return String("ESP32-S3-AI-Assistant-v") + Config::FIRMWARE_VERSION + ".bin";
}
...
static bool otaParseRelease(const String& json, String& outVer, String& outUrl, String& outNotes) {
  ...
  // Auto-derive the expected .bin filename from the remote version tag
  // Pattern: ESP32-S3-AI-Assistant-v<remoteVer>.bin
  String expectedBin = String("\"ESP32-S3-AI-Assistant-v") + outVer + ".bin\"";
  int searchFrom=0;
  while(true) {
    int ni=json.indexOf(expectedBin,searchFrom); if(ni<0) return false;
    int ui=json.indexOf("\"browser_download_url\"",ni);
    if(ui<0||ui-ni>600){searchFrom=ni+1;continue;}
    int uq1=json.indexOf('"',ui+23)+1, uq2=json.indexOf('"',uq1);
    if(uq1>0&&uq2>uq1){outUrl=json.substring(uq1,uq2);return true;}
    searchFrom=ni+1;
  }
}
...
void checkForUpdate(bool silent) {
  ...
  if (!otaParseRelease(json, remoteVer, binUrl, notes)) {
    if (!silent) {
      String expected = String("ESP32-S3-AI-Assistant-v") + remoteVer + ".bin";
      Serial.println("❌ Asset '" + expected + "' not found in latest release.");
      Serial.println("   Make sure the .bin is named exactly: " + expected);
    }
    aiState=AI_IDLE; return;
  }
